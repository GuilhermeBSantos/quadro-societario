...
class PartnerCompanyRepository extends ServiceEntityRepository
{
    ...
    {
        return $this->createQueryBuilder('pa')
            ->select(['p.name', 'p.last_name', 'p.cpf', 'p.phone_number', 'pa.participation'])
    ...
    {
        return $this->createQueryBuilder('pa')
            ->select(['p.fantasy_name', 'p.company_name', 'p.cnpj', 'p.opening_date', 'p.phone_number', 'p.invoicing', 'pa.participation'])
